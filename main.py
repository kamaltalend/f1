...
def load_settings():
    if Load_settings:
        global Force_resolution, Screen, Animations, Mute_volume, Music_volume, Sfx_volume
        try:
            with open('settings.json', 'r') as file:
                file = json.load(file)
...
def update_screen(full_screen=False, rect=None, surf=Window):
    ...
    if rect:
        ...
        if full_screen or len(screen_updates) > 15:  # If in debugging mode then always update entire screen
...
def game():  # All variables that are not constant
    global Track_mask, Player_positions, Race_time, Countdown, Window_screenshot, button_trigger, \
        Screen, Animations, Mute_volume, Music_volume, Sfx_volume, loaded_assets, loaded_sounds, \
...
def main():
    global Player_amount, Npc_amount, Total_laps, loaded_assets, Music_volume, Screen, Sfx_volume, \
